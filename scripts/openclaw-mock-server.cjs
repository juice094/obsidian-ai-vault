...
// 仅用于验证 spike 脚本的协议解析与事件收集逻辑
const http = require('http');
const crypto = require('crypto');

const PORT = process.env.MOCK_PORT || 18791;
const TOKEN = process.env.MOCK_TOKEN || 'mock-admin-token';
...
server.listen(PORT, () => {
  console.log(`Mock OpenClaw server listening on ws://127.0.0.1:${PORT}/ws`);
});
